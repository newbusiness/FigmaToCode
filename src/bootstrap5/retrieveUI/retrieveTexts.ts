...
import { bootstrapNearestColor } from "../builderImpl/bootstrapColor";
...
export const retrieveBootstrapText = (
  sceneNode: Array<AltSceneNode>
): Array<namedText> => {
  // convert to AltNode and then flatten it. Conversion is necessary because of [bootstrapText]
  ...
  selectedText.forEach((node) => {
    if (node.type === "TEXT") {
      const attr = new BootstrapTextBuilder(node, false, false)
        .blend(node)
...
const convertColor = (
  ...
  if (fill?.type === "SOLID") {
    return bootstrapNearestColor(rgbTo6hex(fill.color));
  }
