...
export const bootstrapSolidColor = (fill: SolidPaint, kind: string): string => {
  // don't set text color when color is black (default) and opacity is 100%
  if (
    kind === "text" &&
    fill.color.r === 0.0 &&
 ...
 * example: shadow
 */
export const bootstrapGradientFromFills = (
  fills: ReadonlyArray<Paint> | PluginAPI["mixed"]
): string => {
...
export const bootstrapGradient = (fill: GradientPaint): string => {
  ...
  if (fill.gradientStops.length === 1) {
    ...
    // last
    const toColor = getBootstrapFromFigmaRGB(
...
// AutoGenerated for Bootstrap 2 via [convert_bootstrap_colors.js]
export const bootstrapColorsFull: Record<string, string> = {
  "#000000": "black",
  "#ffffff": "white",
