...
const nearestValue$1 = (goal, array) => {
...
const pixelToTailwindValue = (value, conversionMap) => {
    return conversionMap[nearestValue$1(value / 16, Object.keys(conversionMap).map((d) => +d))];
};
...
const nearestOpacity$1 = (nodeOpacity) => nearestValue$1(nodeOpacity * 100, opacityValues$1);
...
const pxToFontSize$1 = (value) => pixelToTailwindValue(value, mapFontSize$1);
const pxToBorderRadius$1 = (value) => pixelToTailwindValue(value, mapBorderRadius$1);
const pxToLayoutSize$1 = (value) => pixelToTailwindValue(value, mapWidthHeightSize$1);
...
const tailwindSolidColor = (fill, kind) => {
    ...
    const opacityProp = opacity !== 1.0 ? `${kind}-opacity-${nearestOpacity$1(opacity)} ` : "";
...
const tailwindGradient = (fill) => {
    const direction = gradientDirection$3(gradientAngle(fill));
...
const gradientDirection$3 = (angle) => {
    switch (nearestValue$1(angle, [-180, -135, -90, -45, 0, 45, 90, 135, 180])) {
        case 0:
...
const tailwindOpacity = (node) => {
    ...
    if (node.opacity !== undefined && node.opacity !== 1) {
        return `opacity-${nearestOpacity$1(node.opacity)} `;
    }
    return "";
};
/**
...
const tailwindRotation = (node) => {
    ...
    if (node.rotation !== undefined && Math.round(node.rotation) !== 0) {
        ...
        let nearest = nearestValue$1(node.rotation, allowedValues);
        let minusIfNegative = "";
        if (nearest < 0) {
            minusIfNegative = "-";
            nearest = -nearest;
...
const tailwindBorderWidth = (node) => {
    ...
    if (node.strokes && node.strokes.length > 0 && node.strokeWeight > 0) {
        ...
        const nearest = nearestValue$1(node.strokeWeight, allowedValues);
        if (nearest === 1) {
            // special case
            return "border ";
        }
...
const tailwindBorderRadius = (node) => {
    ...
    if (node.cornerRadius !== figma.mixed) {
        if (node.cornerRadius >= node.height / 2) {
            ...
            comp += `rounded${pxToBorderRadius$1(node.cornerRadius)} `;
        }
    }
    else {
        // todo optimize for tr/tl/br/bl instead of t/r/l/b
        if (node.topLeftRadius !== 0) {
            comp += `rounded-tl${pxToBorderRadius$1(node.topLeftRadius)} `;
        }
        if (node.topRightRadius !== 0) {
            comp += `rounded-tr${pxToBorderRadius$1(node.topRightRadius)} `;
        }
        if (node.bottomLeftRadius !== 0) {
            comp += `rounded-bl${pxToBorderRadius$1(node.bottomLeftRadius)} `;
        }
        if (node.bottomRightRadius !== 0) {
            comp += `rounded-br${pxToBorderRadius$1(node.bottomRightRadius)} `;
        }
    }
    return comp;
};
...
const tailwindPosition = (node, parentId = "", hasFixedSize = false) => {
    ...
    if (node.parent.isRelative === true) {
        ...
        return retrieveAbsolutePos$2(node, hasFixedSize);
    }
    return "";
};
const retrieveAbsolutePos$2 = (node, hasFixedSize) => {
...
const tailwindSizePartial = (node) => {
    ...
    if (typeof size.width === "number") {
        w += `w-${pxToLayoutSize$1(size.width)} `;
    }
    ...
    if (typeof size.height === "number") {
        h = `h-${pxToLayoutSize$1(size.height)} `;
    }
...
const tailwindPadding = (node) => {
    ...
    if ("all" in padding) {
        return `p-${pxToLayoutSize$1(padding.all)} `;
    }
    let comp = "";
    // horizontal and vertical, as the default AutoLayout
    if (padding.horizontal) {
        comp += `px-${pxToLayoutSize$1(padding.horizontal)} `;
    }
    if (padding.vertical) {
        comp += `py-${pxToLayoutSize$1(padding.vertical)} `;
    }
    // if left and right exists, verify if they are the same after [pxToLayoutSize] conversion.
    if (padding.left && padding.right) {
        const left = pxToLayoutSize$1(padding.left);
        const right = pxToLayoutSize$1(padding.right);
        if (left === right) {
            comp += `px-${left} `;
        }
        else {
        ...
        comp += `pl-${pxToLayoutSize$1(padding.left)} `;
    }
    else if (padding.right) {
        comp += `pr-${pxToLayoutSize$1(padding.right)} `;
    }
    // if top and bottom exists, verify if they are the same after [pxToLayoutSize] conversion.
    if (padding.top && padding.bottom) {
        const top = pxToLayoutSize$1(padding.top);
        const bottom = pxToLayoutSize$1(padding.bottom);
        if (top === bottom) {
            comp += `py-${top} `;
        }
        else {
        ...
        comp += `pt-${pxToLayoutSize$1(padding.top)} `;
    }
    else if (padding.bottom) {
        comp += `pb-${pxToLayoutSize$1(padding.bottom)} `;
    }
    return comp;
};
...
class TailwindTextBuilder extends TailwindDefaultBuilder {
    ...
    fontSize(node) {
        // example: text-md
        if (node.fontSize !== figma.mixed) {
            const value = pxToFontSize$1(node.fontSize);
...
const htmlGroup = (node, isJsx = false) => {
    ...
    const builder = new HtmlDefaultBuilder(node, showLayerName$2, isJsx)
        .blend(node)
        .widthHeight(node)
...
const htmlBuilder = (node, isJsx, isUI = false) => {
    let builderResult;
    {
        builderResult = new HtmlTextBuilder(node, showLayerName$2, isJsx);
    }
    const splittedChars = node.characters.split("\n");
    const charsWithLineBreak = splittedChars.length > 1
        ? node.characters.split("\n").join("<br/>")
...
const htmlFrame = (node, isJsx = false) => {
    ...
    if (node.layoutMode !== "NONE") {
        const rowColumn = rowColumnProps$2(node, isJsx);
...
const htmlContainer = (node, children, additionalStyle = "", isJsx, isInput = false, isRelative = false) => {
    ...
    const builder = new HtmlDefaultBuilder(node, showLayerName$2, isJsx)
        ...
        .widthHeight(node)
        .autoLayoutPadding(node)
        .position(node, parentId$4, isRelative)
    ...
    if (builder.style || additionalStyle) {
        ...
        else if (selfClosingTags$2.includes(tag) || isJsx) {
            return `\n<${tag}${build}${src} />`;
        }
        else {
            return `\n<${tag}${build}${src}></${tag}>`;
...
const rowColumnProps$2 = (node, isJsx) => {
    // ROW or COLUMN
...
const tailwindGroup = (node, isJsx = false) => {
    ...
    // this needs to be called after CustomNode because widthHeight depends on it
    const builder = new TailwindDefaultBuilder(node, showLayerName$1, isJsx)
        .blend(node)
        .widthHeight(node)
...
const tailwindText = (node, isInput, isJsx) => {
    // follow the website order, to make it easier
    const builderResult = new TailwindTextBuilder(node, showLayerName$1, isJsx)
        .blend(node)
        .textAutoSize(node)
        .position(node, parentId$3)
...
const tailwindFrame = (node, isJsx) => {
    ...
    if (node.layoutMode !== "NONE") {
        const rowColumn = rowColumnProps$1(node);
...
const tailwindContainer = (node, children, additionalAttr, attr, isJsx) => {
    ...
    const builder = new TailwindDefaultBuilder(node, showLayerName$1, isJsx)
        ...
        .widthHeight(node)
        .autoLayoutPadding(node)
        .position(node, parentId$3, attr.isRelative)
    ...
    if (builder.attributes || additionalAttr) {
        ...
        else if (selfClosingTags$1.includes(tag) || isJsx) {
            return `\n<${tag}${build}${src} />`;
        }
        else {
            return `\n<${tag}${build}${src}></${tag}>`;
...
const rowColumnProps$1 = (node) => {
    ...
    // space between items
    const spacing = node.itemSpacing > 0 ? pxToLayoutSize$1(node.itemSpacing) : 0;
 ...
 * Then, find in the corresponding table the closest value.
 */
const pixelToBootstrapValue = (value, conversionMap) => {
...
const pxToFontSize = (value) => pixelToBootstrapValue(value, mapFontSize);
const pxToBorderRadius = (value) => pixelToBootstrapValue(value, mapBorderRadius);
const pxToLayoutSize = (value) => {
 ...
 * example: invisible
 */
const bootstrapVisibility = (node) => {
    // [when testing] node.visible can be undefined
    // When something is invisible in Figma, it isn't gone. Groups can make use of it.
    // Therefore, instead of changing the visibility (which causes bugs in nested divs),
    // this plugin is going to ignore color and stroke
 ...
 * example: border-2
 */
const bootstrapBorderWidth = (node) => {
    // [node.strokeWeight] can have a value even when there are no strokes
    // [when testing] node.effects can be undefined
    if (node.strokes && node.strokes.length > 0 && node.strokeWeight > 0) {
        const allowedValues = [1, 2, 4, 8];
 ...
 * example: rounded-tr-lg
 */
const bootstrapBorderRadius = (node) => {
    if (node.type === "ELLIPSE") {
        return "rounded-full ";
    }
    else if ((!("cornerRadius" in node) && !("topLeftRadius" in node)) ||
...
const bootstrapPosition = (node, parentId = "", hasFixedSize = false) => {
    // don't add position to the first (highest) node in the tree
    if (!node.parent || parentId === node.parent.id) {
        return "";
    }
...
const bootstrapColorFromFills = (fills, kind) => {
    // kind can be text, bg, border...
    // [when testing] fills can be undefined
    const fill = retrieveTopFill(fills);
    if ((fill === null || fill === void 0 ? void 0 : fill.type) === "SOLID") {
        // don't set text color when color is black (default) and opacity is 100%
        return bootstrapSolidColor(fill, kind);
    }
    return "";
};
const bootstrapSolidColor = (fill, kind) => {
    var _a;
    // don't set text color when color is black (default) and opacity is 100%
    if (kind === "text" &&
        fill.color.r === 0.0 &&
    ...
    // example: text-red-500
    const colorProp = `${kind}-${getBootstrapFromFigmaRGB(fill.color)} `;
    // if fill isn't visible, it shouldn't be painted.
    return `${colorProp}${opacityProp}`;
};
/**
 * example: shadow
 */
const bootstrapGradientFromFills = (fills) => {
    // [when testing] node.effects can be undefined
    const fill = retrieveTopFill(fills);
    if ((fill === null || fill === void 0 ? void 0 : fill.type) === "GRADIENT_LINEAR") {
        return bootstrapGradient(fill);
    }
    return "";
};
const bootstrapGradient = (fill) => {
    const direction = gradientDirection(gradientAngle(fill));
    if (fill.gradientStops.length === 1) {
        const fromColor = getBootstrapFromFigmaRGB(fill.gradientStops[0].color);
        return `${direction} from-${fromColor} `;
    }
    else if (fill.gradientStops.length === 2) {
        const fromColor = getBootstrapFromFigmaRGB(fill.gradientStops[0].color);
        const toColor = getBootstrapFromFigmaRGB(fill.gradientStops[1].color);
        return `${direction} from-${fromColor} to-${toColor} `;
    }
    else {
        const fromColor = getBootstrapFromFigmaRGB(fill.gradientStops[0].color);
        // middle (second color)
        const viaColor = getBootstrapFromFigmaRGB(fill.gradientStops[1].color);
        // last
        const toColor = getBootstrapFromFigmaRGB(fill.gradientStops[fill.gradientStops.length - 1].color);
        return `${direction} from-${fromColor} via-${viaColor} to-${toColor} `;
    }
};
...
const bootstrapNearestColor = nearestColorFrom(Object.keys(bootstrapColors));
// figma uses r,g,b in [0, 1], while nearestColor uses it in [0, 255]
const getBootstrapFromFigmaRGB = (color) => {
    const colorMultiplied = {
        r: color.r * 255,
        g: color.g * 255,
        b: color.b * 255,
...
const bootstrapSizePartial = (node) => {
    const size = nodeWidthHeight(node, true);
    let w = "";
    if (typeof size.width === "number") {
 ...
 * https://www.w3schools.com/css/css_dimension.asp
 */
const htmlSizeForBootstrap = (node, isJSX) => {
    return htmlSizePartialForBootstrap(node, isJSX).join("");
};
const htmlSizePartialForBootstrap = (node, isJSX) => {
    return [
        formatWithJSX("width", isJSX, node.width),
        formatWithJSX("height", isJSX, node.height),
    ];
...
/**
 * https://bootstrapcss.com/docs/margin/
 * example: px-2 py-8
 */
const bootstrapPadding = (node) => {
    const padding = commonPadding(node);
    if (!padding) {
        return "";
    }
...
class BootstrapDefaultBuilder {
    ...
    widthHeight(node) {
        ...
        if ("isRelative" in node && node.isRelative === true) {
            ...
            const [htmlWidth, htmlHeight] = htmlSizePartialForBootstrap(node, this.isJSX);
            // when textAutoResize is NONE or WIDTH_AND_HEIGHT, it has a defined width.
            if (node.type !== "TEXT" || node.textAutoResize !== "WIDTH_AND_HEIGHT") {
                if (node.width > 384) {
                    this.style += htmlWidth;
...
class BootstrapTextBuilder extends BootstrapDefaultBuilder {
    constructor(node, showLayerName, optIsJSX) {
        super(node, showLayerName, optIsJSX);
    }
    // must be called before Position method
    ...
    tryGetGlobalStyleFromFont(thisStyle) {
...
const bootstrapMain = (sceneNode, parentIdSrc = "", isJsx = false, layerName = false) => {
...
// todo lint idea: replace BorderRadius.only(topleft: 8, topRight: 8) with BorderRadius.horizontal(8)
const bootstrapWidgetGenerator = (sceneNode, isJsx) => {
    let comp = "";
    // filter non visible nodes. This is necessary at this step because conversion already happened.
    const visibleSceneNode = sceneNode.filter((d) => d.visible !== false);
    visibleSceneNode.forEach((node) => {
        if (node.type === "RECTANGLE" || node.type === "ELLIPSE") {
            comp += bootstrapContainer(node, "", "", { isRelative: false, isInput: false }, isJsx);
        }
        else if (node.type === "GROUP") {
...
const bootstrapGroup = (node, isJsx = false) => {
    // ignore the view when size is zero or less
    // while technically it shouldn't get less than 0, due to rounding errors,
    // it can get to values like: -0.000004196293048153166
    // also ignore if there are no children inside, which makes no sense
...
const bootstrapText = (node, isInput, isJsx) => {
    // follow the website order, to make it easier
...
const bootstrapFrame = (node, isJsx) => {
    ...
    if (node.children.length === 1 &&
        ...
        return bootstrapContainer(node, ` placeholder="${char}"`, attr, { isRelative: false, isInput: true }, isJsx);
    }
    const childrenStr = bootstrapWidgetGenerator(node.children, isJsx);
    if (node.layoutMode !== "NONE") {
        const rowColumn = rowColumnProps(node);
        return bootstrapContainer(node, childrenStr, rowColumn, { isRelative: false, isInput: false }, isJsx);
    }
    // node.layoutMode === "NONE" && node.children.length > 1
    // children needs to be absolute
    return bootstrapContainer(node, childrenStr, "relative ", { isRelative: true, isInput: false }, isJsx);
};
// properties named propSomething always take care of ","
// sometimes a property might not exist, so it doesn't add ","
const bootstrapContainer = (node, children, additionalAttr, attr, isJsx) => {
    var _a;
    // ignore the view when size is zero or less
    // while technically it shouldn't get less than 0, due to rounding errors,
    // it can get to values like: -0.000004196293048153166
...
const rowColumnProps = (node) => {
    ...
    node.layoutMode === "HORIZONTAL" ? "x" : "y";
    // space is visually ignored when there is only one child or spacing is zero
    let space = "";
